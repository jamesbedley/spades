function parseSeed(seedString) {
    let seed = parseInt(seedString, 10);
    if (isNaN(seed)) {
        console.log ("***no BED seed!");
        seed = 2768;
    }
    return seed;
}
function bedGetSeed(){
    let searchParamz = new URLSearchParams(window.location.search);
    const seedString = searchParamz.get("seed");
    let seed = parseSeed(seedString);
    console.log ("it worked: " + seed);
    const seedIn = document.getElementById("seedNum");
    seedIn.value = seed;
    return seed;
}
const seedIn = document.getElementById("seedNum");
seedIn.addEventListener("change", e => {
    console.log ("this is seedIn: " + seedIn.value);
    let seed = parseSeed(seedIn.value);
    let searchParamz = new URLSearchParams(window.location.search);
    searchParamz.set("seed", seed);
    window.location.search = searchParamz.toString();
});

function shouldBidNil(dodgedata, hand, contigSpades) {
    const nonTrump = dodgedata.filter(d => d.suit != "s");
    let okSoFar = nonTrump.every(d => (d.dodgers >= 3 && d.dodgeDistance >= 5) || d.dodgeDistance >= 6);
    if (!okSoFar) return (false);
    const trump = dodgedata.find (d => d.suit == "s");
    okSoFar = trump.dodgers < 0 && trump.dodgeDistance >= 7;
    return okSoFar;
}
...
! function r(s, o, l) {
        ...
        var i = e("../helper-functions"),
            ...
            s = function() {
                ...
                return t.makeRenderFunc = function(a) {
                    ...
                    0 <= this.lastDealerIndex ? this.dealerIndex = (this.lastDealerIndex + 1) % e.length : this.dealerIndex = 1, this.nextPlayerToDealTo = (this.dealerIndex + 1) % e.length, this.renderEvent("pickdealer", function() {}, {
                    ...
                    if (this.allCards = this.deck.slice(), this.deckCode) {
                        ...
                        this.deck = n
                    }   //      this is the seed vvvv  2768 is good - can do a zero bid?
                    else this.shuffle(this.deck, bedGetSeed());
    ...
    4: [function(e, t, a) {
        ...
        h.prototype.rotate = function(e, t) {
        ...
        }, h.prototype.normalizeRotationOnMove = !0, h.prototype.showCard = function(e,skip) {
            if (!skip) {
                this.bedhidden = false;
            }
            this.normalizeRotationOnMove && this.rotate(0);
            var t = this.suit.toLowerCase() + this.rank;
            if (skip){
                return $(this.guiCard).addClass(t).addClass("up"), this
            }
            else {
        ...
        }, h.prototype.hideCard = function(e,skip) {
            if (!skip) {
                this.bedhidden = true;
            }
            var t = this.suit.toLowerCase() + this.rank;
            if (bedcheck) {
                ...
                this.normalizeRotationOnMove && this.rotate(0);
                if (skip) {
                    return $(this.guiCard).removeClass("up").removeClass(t), this;
                }
                else {
        ...
        var S = function() {
            ...
            return t.setBoardHeight = function(e) {
              ...
              // "left:" values are all wrong, derived from P.TABLE_SIZE which is also wrong, check to see why
                ...
                for (var n = 0; n < e.length; n++) {
                    ...
                    (n + 1) % P.CONDENSE_COUNT == 0 && (a -= P.OVERLAY_MARGIN, t -= P.OVERLAY_MARGIN), this._createGuiCard(r, {
                        left: a,
                        top: t
                        // ? means if i is true do the first thing, else do the second thing, i is treated as a boolean
                ...
                var a = {};
                var widthAdjust = 0; // when the bottom player's bid is 10 or more have to shift the trick count 9 px to the left
                if (e.position === P.BOTTOM) {
                    console.log("bottom player t.left is " + JSON.stringify(t.left) + " width is " + JSON.stringify($("#bottom-player-trick-count").width()) + " trick count " + JSON.stringify(e.bidValue))
                    if (e.bidValue > 9) {
                        widthAdjust = 9
                    }
                }
                return e.position === P.TOP ? (a.top = t.top, a.left = t.left + P.TRICK_SIZE.width) :
                       e.position === P.BOTTOM ? (a.top = t.top, a.left = t.left - widthAdjust - $("#bottom-player-trick-count").width()) :
                       e.position === P.LEFT ? (a.top = t.top - $("#left-player-trick-count").height(), a.left = t.left) :
                       e.position === P.RIGHT && (a.left = t.left, k.isMobileLookActive() ? a.top = t.top - $("#right-player-trick-count").height() : a.top = t.top + P.TRICK_SIZE.width), a
                ...
                d(function() {
                    ...
                    console.log ("h.player.id is " + JSON.stringify(h.player.id) + " bid value is " + JSON.stringify(h.player.bidValue)) // ***********************************
            ...
            n = function() {
                ...
                return t.toString = function() {
                    ...
                    if (13 !== this.partner.bidValue) {
                        ...
                        console.log("count of hand: " + JSON.stringify(t));
                        ...
                        // at this point take care of gaps in top spades like AQx for 2; KQx for two; etc
                        // this list could get lengthy - have to integrate roughing value too
                        var topSpades = l;
                        // l (ie. small L) is the count of contiguous top spades
                        R.debug("********** " + this.name + " BID ***************"), R.debug(this.name + ": Hard Bid is " + l);
                        var spadesForTrumping = t["s"].count - topSpades;
                        console.log("extra spades: " + JSON.stringify(spadesForTrumping));
                        // maybe just count voids, singletons and doubletons instead of...

                        function countNaturalWinners (suit, hand) {
                            // => is called lambda which replaces a function.  ls of lambda is parameter, rs is the body of the function
                            // without {} the return for the function (ie. the lambda) is implied.
                            // .filter is a function that searches an array (eg. hand) for whatever condition follows
                            const cardsOfSuit = hand.filter(card => card.suit === suit);
                            let naturalWinners = 0;
                            for (let value = 14; value > 14 - cardsOfSuit.length; value--) {
                                // .filter returns an array but .some returns a boolean
                                if (cardsOfSuit.some(card => card.rank === value)) {
                                    naturalWinners++;
                                }
                            }
                            return naturalWinners;
                        }

                        function makeRemainder (cards) {
                            const remainder =[];
                            for (let rank = 2; rank <= 14; rank++) {
                                if (!cards.includes(rank)) {
                                    remainder.push(rank);
                                }
                            }
                            return remainder;
                        ...
                        function dodgeCount(suit, hand) {
                            let dodgers = 0;
                            const cardsOfSuit = hand
                                .filter(card => card.suit === suit)
                                .map(card => card.rank);
                            cardsOfSuit.sort((a,b) => a-b);
                            let ri = 0;
                            const remainder = makeRemainder(cardsOfSuit).reverse();
                            let dodgeDistance = 0;                                    // needs to be a sum and eventually an average
                            let dodgeDistanceCount = 0;
                            for (let i = 0; i < cardsOfSuit.length; i++) {
                              const rank = cardsOfSuit[i];
                              const highestRemaining = remainder[ri];

                                // console.log("rank " + rank + " hr " + highestRemaining);
                              if (rank > highestRemaining) {
                                  if (dodgeDistanceCount == 0) {
                                    dodgeDistanceCount++;
                                  }
                                return {dodgers, dodgeDistance:dodgeDistance / dodgeDistanceCount};
                              } else {
                                dodgeDistance += highestRemaining - rank;
                                dodgeDistanceCount ++;
                                dodgers++;
                                ri++;
                              }
                            ...
                            if (dodgeDistanceCount == 0) {
                                dodgeDistanceCount = 1;
                                dodgeDistance = MAX_DISTANCE;
                              }
                            return {dodgers:MISS, dodgeDistance:dodgeDistance / dodgeDistanceCount};
                          }


                        const dodgedata = ["s","h","d","c"].map(suit => {
                            let something = dodgeCount(suit,this.hand);
                            console.log ("the dodge count for " + suit + " is " + something.dodgers + " distance " + something.dodgeDistance);
                            something.suit = suit;
                            return something;
                        });

                        let bedShouldBidNil = shouldBidNil(dodgedata);

                        const naturalSpadeWinners = countNaturalWinners("s",this.hand);
                        console.log("naturalSpadeWinners: " + naturalSpadeWinners);
                        let roughingBids = 0;
                        if (spadesForTrumping > 0) {
                            if (t["c"].count < 2) {
                                let roughs = roughingBids + 2 - t["c"].count;
                                if (roughs > spadesForTrumping) { roughs = spadesForTrumping };
                                roughingBids = roughingBids + roughs;
                                spadesForTrumping = spadesForTrumping - roughs;
                            }
                        }
                        if (spadesForTrumping > 0) {
                            if (t["d"].count < 2) {
                                let roughs = roughingBids + 2 - t["d"].count;
                                if (roughs > spadesForTrumping) { roughs = spadesForTrumping };
                                roughingBids = roughingBids + roughs;
                                spadesForTrumping = spadesForTrumping - roughs;
                            }
                        ...
                        if (spadesForTrumping > 0) {
                            if (t["h"].count < 2) {
                                let roughs = roughingBids + 2 - t["h"].count;
                                if (roughs > spadesForTrumping) { roughs = spadesForTrumping };
                                roughingBids = roughingBids + roughs;
                                spadesForTrumping = spadesForTrumping - roughs;
                            }
                        ...
                        for (f = g ? f : f[Symbol.iterator]();;) {
                            ...
                            var v = b;
                            // _. is a library of functions and _.remove removes v from o
                            d++, _.remove(o, v)
                        }
                        // not sure what k represents, ie. low spades? It does NOT mean all spades lower than 10!
                        var k = Math.ceil(o.length / 2.5);
                        // eg. Bill has S: 2,3,5,10 and gets a k value of 2
                        for (var w in R.debug(this.name + ": Low spade bid is " + k), d += k, t)
                            if (w !== D) {
                                var C = t[w];
                                C.hasAce && C.hasKing ? (d += 2, R.debug(this.name + " Bid for King+Ace in " + w + ": 2")) :
                                C.hasAce ? (d += 1, R.debug(this.name + " Bid for Ace in " + w + ": 1")) :
                                C.hasKing && 2 <= o.length && (d += .5, R.debug(this.name + " Bid for King in " + w + ": 0.5"))
                            } var S = Math.min(1, d);
                            // d is the calculated soft bid but why is the soft bid being lowered by at minimum 1?
                            // It DOES generate the correct bid however in our example
                            R.debug(this.name + " lowering soft bid by " + S);
                        var T = l + (d -= S);
                        if (R.debug(this.name + " Soft+Hard Bid: " + T), this.shouldBidNil(T, m, t)) {
                            console.log(`Bot thinks we should bid nil. We think we ${bedShouldBidNil ? 'should' : 'shouldn\t'}.`);
                            this.game.bid(this, 0);
                        }
                        else {
                            if (bedShouldBidNil) {
                              console.log('Bot is too chicken to bid nil, luckily we\'re in charge');
                              this.game.bid(this, 0);
                              return;
                            }
                            T = Math.ceil(T), T = Math.min(T, 7), T = Math.max(T, 1);
                            if (7 <= this.partner.bidValue) R.debug(this.name + " changing bid from " + T + " to hard bid " + l + " because our partner has bid 6 or more"), T = l;
                            ...
                            // this is where the bid is forced to at least 1 whenever a NIL bid is not made
                    ...
                    // I don't like this - leave the bid alone because it leads to excessive bags
                ...
                return t.toString = function() {
                    ...
                    for (e = t ? e : e[Symbol.iterator]();;) {
                        ...
                        if ("bottom-player" === n.id && !this.handSorted) {
                            ...
                            this.sortHand(e[1], ()=>{});
                            this.sortHand(e[2], ()=>{});
                            return this.sortHand(e[3], this.afterDealing);
                        }
...
document.getElementById("allseeing").addEventListener("change",e => {
    console.log("god mode is: " + (e.target.checked ? "on":"off"));
    console.log(bedcards);
    bedcheck = e.target.checked;
    if (e.target.checked) {
        for (let i = 0; i < bedcards.length; i++)
        {
            bedcards[i].showCard(undefined,true);
        }
        ...
        for (let i = 0; i < bedcards.length; i++)
        {
            if (bedcards[i].bedhidden){
                bedcards[i].hideCard(undefined,true);
            }
        }
    }
